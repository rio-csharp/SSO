...
当前仓库已进入阶段 4，在前几个阶段的基础上，开始接入 PostgreSQL/EF Core 持久化基础。

本阶段目标：

- 保持 Domain / Contracts / Application 内核稳定
- 建立 EF Core DbContext、实体映射与仓储实现
- 为 Web / Api 宿主接入 PostgreSQL 持久化注册入口
- 继续通过测试固定分层边界、配置校验与持久化基础行为
...
- Tests：覆盖邮箱校验、客户端安全约束、会话撤销规则、应用层处理器行为、基础设施注入与仓储基础行为

## 后续阶段

后续将按增量方式推进：

1. 集成 OpenIddict 与身份系统
2. 实现认证核心
3. 完善审计、会话、后台与客户端接入
4. 完成联调、加固与全链路验收
